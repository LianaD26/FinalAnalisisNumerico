...
class ILagrange:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Polinomio de Lagrange")
    ...
    def calcular(self):
        try:
            ...
            pol = InterAjusteController.lagrange_controller(x_vals, y_vals)
            self.result_label.config(text=f"El polinomio es: {pol} \n"
                                          f"f({value}): "
                                          f"{InterAjusteController.aprox_controller_sympy(pol, float(value))}")
